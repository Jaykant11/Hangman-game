/* 
 ...
 to keep its options open.  You can change the setting for SHOW_COUNT to be "true" 
 to see how many options are still left on each turn.
 
 The game will ask what length of word the user wants to use, and then prune the
 dictionary to only include words of that length. Then the program will ask how
 many wrong answers the user is allowed to guess. Finally the game starts. With each
 incorrect guess the the number of guesses allowed will decrease by one, while a
 correct guess will not affect the guesses still allowed. The program works by attempting
 to put a the guessed letter into each of the availiable spaces, and creating potential
 libraries of words with each position of the letter, and then choosing which library is 
 the largest and continues to prune the library until it absolutely has to choose a word.
*/
